...
Hãy trả lời một cách CỰC KỲ ngắn gọn, súc tích, đi thẳng vào vấn đề chính. Không chào hỏi rườm rà, không giải thích dông dài để tiết kiệm tối đa token sử dụng. Định dạng câu trả lời bằng Markdown nếu cần thiết (dùng in đậm, gạch đầu dòng, vv).`;
  ...
  if (doc) {
    ...
    if (doc.fileSize > 15 * 1024 * 1024) {
      console.log(`[AI Chat Service] File size (${doc.fileSize} bytes) exceeds 15MB limit.`);
      ...
      console.log(`[AI Chat Service] File type analysis: isTextFile=${isTextFile}, isPdf=${isPdf}, isImage=${isImage}, MimeType="${mimeType}"`);

      if (isTextFile || isPdf || isImage) {
        try {
          console.log(`[AI Chat Service] Fetching file stream from S3 for Key: "${doc.s3Key}"...`);
          ...
          console.log(`[AI Chat Service] Successfully read ${fileBuffer.length} bytes from S3 stream.`);
          ...
          if (isTextFile) {
            ...
            console.log(`[AI Chat Service] Appended text file content context (${truncatedText.length} chars).`);
            ...
            console.log(`[AI Chat Service] Prepared multimodal filePart for Gemini (base64 length: ${filePart.inlineData.data.length}).`);
          }
        } catch (err) {
          console.error('[AI Chat Service] Error fetching file for AI context:', err);
          fileContextPrompt = `\n[Lỗi: Không thể tải nội dung tệp "${doc.fileName}" từ S3]`;
        }
      } else {
        console.log(`[AI Chat Service] File format "${mimeType}" is not supported for reading.`);
  ...
  console.log(`[AI Chat Service] Sending request to Gemini API. Payload parts count: ${userParts.length}. Has filePart: ${!!filePart}`);
  ...
  if (!response.ok) {
    ...
    console.error('[AI Chat Service] Gemini API Error:', errorData);
    throw new Error('Rất tiếc, tôi gặp sự cố khi kết nối tới máy chủ trí tuệ nhân tạo. Vui lòng thử lại sau!');
  }

  const data = await response.json();
  const reply = data.candidates?.[0]?.content?.parts?.[0]?.text || 'Tôi không nhận được phản hồi phù hợp từ AI.';
  console.log(`[AI Chat Service] Success! Gemini replied with ${reply.length} characters.`);
  return reply;
};
